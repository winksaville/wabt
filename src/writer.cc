...
Result OutputBuffer::WriteToFile(const char* filename) {
...
std::unique_ptr<OutputBuffer> MemoryWriter::ReleaseOutputBuffer() {
  return std::unique_ptr<OutputBuffer>(buf_.release());
}

FileWriter::FileWriter(FILE* file) : file_(file), offset_(0) {}
