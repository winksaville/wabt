...
class BinaryWriter {
 ...
 public:
  BinaryWriter(Writer* writer, const WriteBinaryOptions* options);
...
BinaryWriter::BinaryWriter(Writer* writer, const WriteBinaryOptions* options)
    : stream_(writer, options->log_stream),
      log_stream_(options->log_stream),
      options_(options) {}

void BinaryWriter::WriteHeader(const char* name, int index) {
  if (log_stream_) {
    if (index == PRINT_HEADER_NO_INDEX) {
      log_stream_->Writef("; %s\n", name);
    } else {
      log_stream_->Writef("; %s %d\n", name, index);
    }
  }
}
...
static void write_inline_signature_type(Stream* stream_,
                                        const BlockSignature& sig) {
  if (sig.size() == 0) {
    write_type(stream_, Type::Void);
  } else if (sig.size() == 1) {
    write_type(stream_, sig[0]);
  } else {
    /* this is currently unrepresentable */
    stream_->WriteU8(0xff, "INVALID INLINE SIGNATURE");
  }
}
...
void BinaryWriter::WriteExpr(const Module* module,
  ...
  switch (expr->type) {
    ...
    case ExprType::BrTable: {
      ...
      write_u32_leb128(&stream_, expr->br_table.targets->size(),
                       "num targets");
      uint32_t depth;
...
void BinaryWriter::WriteLimits(const Limits* limits) {
  uint32_t flags = limits->has_max ? WABT_BINARY_LIMITS_HAS_MAX_FLAG : 0;
  write_u32_leb128(&stream_, flags, "limits: flags");
  write_u32_leb128(&stream_, limits->initial, "limits: initial");
  if (limits->has_max)
    write_u32_leb128(&stream_, limits->max, "limits: max");
}
...
Result BinaryWriter::WriteModule(const Module* module) {
  ...
  if (module->imports.size()) {
    ...
    for (size_t i = 0; i < module->imports.size(); ++i) {
      ...
      write_str(&stream_, import->module_name.start,
                import->module_name.length, PrintChars::Yes,
                "import module name");
      write_str(&stream_, import->field_name.start,
                import->field_name.length, PrintChars::Yes,
                "import field name");
      stream_.WriteU8Enum(import->kind, "import kind");
      switch (import->kind) {
        case ExternalKind::Func:
          write_u32_leb128(&stream_, get_func_type_index_by_decl(
  ...
  if (module->elem_segments.size()) {
    ...
    for (size_t i = 0; i < module->elem_segments.size(); ++i) {
      ...
      write_u32_leb128(&stream_, segment->vars.size(),
  ...
  if (options_->write_debug_names) {
    ...
    if (named_functions > 0) {
      ...
      for (size_t i = 0; i < module->funcs.size(); ++i) {
        ...
        write_str(&stream_, func->name.start, func->name.length,
                  PrintChars::Yes, desc);
      }
      EndSubsection();
    }
    ...
    for (size_t i = 0; i < module->funcs.size(); ++i) {
      ...
      for (size_t j = 0; j < num_params; ++j) {
        ...
        write_str(&stream_, name.data(), name.length(), PrintChars::Yes,
                  desc);
      }
